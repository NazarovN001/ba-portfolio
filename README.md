...
- запускать n8n локально в Docker на АРМ,
- настраивать VPN-доступ и работать через защищенное сетевое соединение,
- писать промпты для создания сложного поведения ИИ,
- работать с LLM-моделями через OpenAI-совместимый API,
- упаковывать работу ИИ-агентов в пользовательский интерфейс Telegram-бота,
...
Для выполнения заданий этого проекта тебе необходимо:

- Настроить VPN-подключение на АРМ через клиент Throne.
- Запустить n8n локально в Docker.
- Подключить языковую модель GigaChat через OpenAI-совместимый API.

Может показаться логичным и удобным использовать полностью готовый облачный сервис n8n, где не нужно тратить время на настройку. 
...
Для доступа к необходимым сервисам и внешним AI-моделям тебе нужно настроить VPN-подключение.

1. Установи VPN-клиент Throne согласно инструкции (папка materials).
2. Запусти приложение Throne.
3. Скачай профиль маршрутизации Bypass Russia.
4. Импортируй конфигурацию своего VPN-сервера.
5. Включи Tun Mode.
6. Подключись к VPN-серверу.
7. Убедись, что соединение активно.

Важно:

Перед завершением работы обязательно отключай VPN через команду Stop внутри клиента Throne.


### Запуск n8n

После подключения VPN необходимо развернуть локальный экземпляр n8n.

1. Установи Docker.
2. Запусти контейнер n8n.
3. Открой веб-интерфейс n8n.
4. Создай учетную запись администратора.

Цель этапа — получить полностью рабочий локальный экземпляр n8n.


### Подключение GigaChat

Для работы с языковыми моделями используется GigaChat через OpenAI-совместимый API.

1. Получи endpoint и API-ключ проекта.
2. Создай OpenAI Credentials в n8n.
3. Настрой подключение к модели GigaChat.
4. Создай тестовый workflow.
5. Отправь модели сообщение «Привет».
6. Получи ответ модели.

![LLM](misc/images/work_with_LLM.png)

Мощные LLM, такие как GigaChat, требуют значительных вычислительных ресурсов.
Использование API позволяет сосредоточиться на разработке логики агента и не заниматься самостоятельным обучением и сопровождением больших моделей.
На данном этапе ты работаешь напрямую с AI-моделью через n8n без интеграции с Telegram.
...
1. Скриншот активного VPN-подключения в Throne.
2. Скриншот интерфейса n8n.
3. Скриншот workflow.
4. Скриншот ответа модели GigaChat на запрос пользователя.
...
- В качестве AI-модели используй модель GigaChat 2 через OpenAI-совместимое подключение, настроенное в n8n.  для суммаризации отдельного поста и ее же для общей суммаризации всех появившихся постов.
...
В рамках проекта доступ к внешним AI-сервисам осуществляется через VPN-подключение. 

Перед выполнением заданий убедись, что:

  - VPN активен;
  - Tun Mode включен;
  - маршрутизация работает корректно;
  - n8n имеет доступ к необходимым внешним API.

При возникновении сетевых ошибок в первую очередь проверь состояние VPN-подключения.
...
- Для создания контента под пост AI Agent должен использовать интеграцию с tavily_search. Для работы необходимо зарегистрироваться в сервисе и получить credentials. Агент будет делать поиск в интернете и из найденных материалов готовить контент.
...
![Scheme](misc/images/Scheme.jpg)

Да, схема может показаться громоздкой, но только пройдя эти шаги ты на практике, а не в теории поймёшь, как на самом деле под капотом работает нода AI Agent и насколько она удобна, если использовать модель, которая умеет применять tool_calls самостоятельно.
...
Модель GigaChat 2 внутри ноды OpenAI Chat Model, подключенной к AI Agent, на текущий момент не умеет работать с tool_calls, включая tavily_search.
Выходом из этого является следующая цепочка.

**1.** В ноде AI Agent нужно прописать system prompt:

   *Ты — ИИ-агент внутри n8n.*
   *Если для ответа нужны актуальные данные из интернета или новости, ты НЕ отвечаешь сразу.*
   *Вместо этого ты пишешь строку ровно в формате tavily(query="..."), где в кавычках — краткий поисковый запрос.*
...
*Ты получаешь результаты веб-поиска Tavily в JSON.*
