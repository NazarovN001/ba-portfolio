...
1. Запусти приложение Throne (если не запущено) через меню приложений.
2. Скачать профиль маршрутизации "Bypass Russia" (если ранее не скачан).
3. Импортируй конфигурацию VPN-сервера (через Ctrl+V или Ctrl+N).
4. Включи Tun-Mode.
5. Выбери сервер и запусти подключение.
6. Проверь статус.
